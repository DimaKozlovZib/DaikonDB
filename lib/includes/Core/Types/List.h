...
struct ListObject : public BaseDataObject {
    ...
    explicit ListObject(mem::IMemoryTracker* tracker)
        : BaseDataObject(DataType::kList),
          values(Allocator(tracker, this)) {}

    int64_t PredictPushDelta(const std::vector<std::string_view>& vals) const {
        int64_t delta = 0;
    ...
    void PushLeft(const std::vector<std::string_view>& vals) {
        for (auto it = vals.rbegin(); it != vals.rend(); ++it)
            values.emplace_front(it->data(), it->size(), values.get_allocator());
    }
    ...
    int64_t PredictSetDelta(int64_t index, std::string_view new_val) const {
        if (index < 0 || static_cast<size_t>(index) >= values.size()) return 0;
        const auto& old = values[static_cast<size_t>(index)];
        size_t old_cap = old.capacity();
        size_t new_size = new_val.size();
        if (new_size <= old_cap) return 0;
        return static_cast<int64_t>(new_size - old_cap);
    }
    ...
    int64_t PredictInsertDelta(std::string_view val) const {
        return static_cast<int64_t>(val.size() + 32);
    }
