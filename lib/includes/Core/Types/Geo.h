...
struct GeoObject final : public BaseDataObject {
    ...
    std::unordered_map<TrackingString, GeoPoint, std::hash<TrackingString>, std::equal_to<TrackingString>, Allocator> points;

    explicit GeoObject(mem::IMemoryTracker* tracker)
        : BaseDataObject(DataType::kGeo),
          points(Allocator(tracker, &allocated_size)) {}

    GeoObject* AsGeo() override { return this; }

    void Add(double lon, double lat, std::string_view member) {
        points.try_emplace(TrackingString(member.data(), member.size(), points.get_allocator()), GeoPoint{lon, lat});
    }
    ...
    std::vector<std::optional<std::pair<double, double>>> Pos(const std::vector<std::string_view>& members) const {
        ...
        for (const auto& m : members) {
            auto it = points.find(TrackingString(m.data(), m.size(), points.get_allocator()));
            if (it != points.end())
    ...
    std::optional<double> Dist(std::string_view member1, std::string_view member2, std::string_view unit = "m") const {
        auto it1 = points.find(TrackingString(member1.data(), member1.size(), points.get_allocator()));
        auto it2 = points.find(TrackingString(member2.data(), member2.size(), points.get_allocator()));
    ...
    std::vector<std::string_view> Search(double lon, double lat, double radius, std::string_view unit, bool asc = false, std::optional<int64_t> count = std::nullopt) const {
        ...
        auto comp = [asc](const auto& a, const auto& b) { return asc ? a.second < b.second : a.second > b.second; };
    ...
    std::vector<std::pair<std::string_view, GeoPoint>> SearchPoints(double lon, double lat, double radius, std::string_view unit, bool asc = false, std::optional<int64_t> count = std::nullopt) const {
        std::vector<std::pair<std::string_view, std::pair<double, GeoPoint>>> tmp;
        for (const auto& [name, pt] : points) {
            double d = detail::Haversine(lon, lat, pt.longitude, pt.latitude, unit);
            if (d <= radius) tmp.emplace_back(std::string_view(name.data(), name.size()), std::make_pair(d, pt));
        }
    ...
    struct MemoryView {
        ...
        int64_t EstimateAdd(std::string_view member) const {
            return member.size() + sizeof(GeoPoint) + 48;
        }
