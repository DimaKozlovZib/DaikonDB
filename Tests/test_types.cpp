...
TEST_F(TypesTest, StringObject_CreateAndGet) {
    StringObject str(db_.GetMemoryTracker(), "hello");
...
TEST_F(TypesTest, ListObject_PushLeftRight) {
    ListObject list(db_.GetMemoryTracker());
    list.PushLeft({"a", "b"});
    list.PushRight({"c", "d"});
...
TEST_F(TypesTest, ListObject_Pop) {
    ...
    std::vector<std::string> expected_left = {"1", "2"};
    EXPECT_EQ(left, expected_left);
    EXPECT_EQ(list.Len(), 2);
    auto right = list.PopRight(1);
    std::vector<std::string> expected_right = {"4"};
...
TEST_F(TypesTest, ListObject_GetByIndex) {
    ...
    EXPECT_EQ(list.GetByIndex(0).value(), "a");
    EXPECT_EQ(list.GetByIndex(-1).value(), "c");
...
TEST_F(TypesTest, ListObject_Set) {
    ...
    list.PushRight({"a", "b"});
    list.Set(0, "new");
    EXPECT_EQ(list.GetByIndex(0).value(), "new");
}
...
TEST_F(TypesTest, SetObject_AddAndContains) {
    SetObject set(db_.GetMemoryTracker());
...
TEST_F(TypesTest, SetObject_AllMembers) {
    ...
    std::vector<std::string> expected = {"a", "m", "z"};
    EXPECT_EQ(members, expected);
}

TEST_F(TypesTest, SetObject_UnionSets) {
    SetObject set1(db_.GetMemoryTracker()), set2(db_.GetMemoryTracker());
    set1.Add({"a", "b"});
    set2.Add({"b", "c"});
...
TEST_F(TypesTest, SetObject_MoveMember) {
    SetObject src(db_.GetMemoryTracker()), dst(db_.GetMemoryTracker());
    src.Add({"x", "y"});
    dst.Add({"z"});
...
TEST_F(TypesTest, GeoObject_AddAndPos) {
    GeoObject geo(db_.GetMemoryTracker());
...
TEST_F(TypesTest, GeoObject_SearchPoints) {
    ...
    auto res = geo.SearchPoints(10.0, 20.0, 50.0, "km", true);
    ASSERT_EQ(res.size(), 2);

    EXPECT_EQ(res[0].first, "p1");
    EXPECT_DOUBLE_EQ(res[0].second.longitude, 10.0);
    EXPECT_DOUBLE_EQ(res[0].second.latitude, 20.0);
    EXPECT_EQ(res[1].first, "p2");
    EXPECT_DOUBLE_EQ(res[1].second.longitude, 10.1);
    EXPECT_DOUBLE_EQ(res[1].second.latitude, 20.1);
}

TEST_F(TypesTest, BaseDataObject_TtlExpire) {
    StringObject str(db_.GetMemoryTracker(), "ttl_test");
...
TEST_F(TypesTest, MemoryViews_Coverage) {
    StringObject str(db_.GetMemoryTracker(), "test");
